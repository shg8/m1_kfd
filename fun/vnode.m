...
uint64_t funVnodeChownByPtr(uint64_t vnode, char *filename, uid_t uid, gid_t gid) {
    uint64_t v_data = kread64(vnode + off_vnode_v_data);
    uint32_t v_uid = kread32(v_data + 0x80);
    uint32_t v_gid = kread32(v_data + 0x84);
...
uint64_t funVnodeChmodByPtr(uint64_t vnode, char* filename, mode_t mode) {
    uint64_t v_data = kread64(vnode + off_vnode_v_data);
    uint32_t v_mode = kread32(v_data + 0x88);
...
uint64_t findRootVnode(void) {
    ...
    uint64_t textvp = textvp_pac | 0xfffff00000000000;
    ...
    uint64_t devvp = kread64((kread64(textvp + off_vnode_v_mount) | 0xfffff00000000000) + off_mount_mnt_devvp);
    ...
    uint64_t sbin_vnode = kread64(textvp + off_vnode_v_parent) | 0xfffff00000000000;
    textvp_nameptr = kread64(sbin_vnode + off_vnode_v_name);
    textvp_name = kread64(textvp_nameptr);
    devvp = kread64((kread64(textvp + off_vnode_v_mount) | 0xfffff00000000000) + off_mount_mnt_devvp);
    ...
    uint64_t root_vnode = kread64(sbin_vnode + off_vnode_v_parent) | 0xfffff00000000000;
    textvp_nameptr = kread64(root_vnode + off_vnode_v_name);
    textvp_name = kread64(textvp_nameptr);
    devvp = kread64((kread64(root_vnode + off_vnode_v_mount) | 0xfffff00000000000) + off_mount_mnt_devvp);
    nameptr = kread64(devvp + off_vnode_v_name);
    name = kread64(nameptr);
    devName = &name;
...
uint64_t funVnodeRedirectFolder(char* to, char* from) {
    ...
    uint64_t to_devvp = kread64((kread64(to_vnode + off_vnode_v_mount) | 0xfffff00000000000) + off_mount_mnt_devvp);
    uint64_t from_devvp = kread64((kread64(from_vnode + off_vnode_v_mount) | 0xfffff00000000000) + off_mount_mnt_devvp);
...
uint64_t funVnodeOverwriteFile(char* to, char* from) {
    ...
    uint64_t fileglob = fileglob_pac | 0xfffff00000000000;
    uint64_t vnode_pac = kread64(fileglob + off_fg_data);
    uint64_t to_vnode = vnode_pac | 0xfffff00000000000;
    printf("[i] %s to_vnode: 0x%llx\n", to, to_vnode);
    
    uint64_t rootvnode_mount_pac = kread64(findRootVnode() + off_vnode_v_mount);
    uint64_t rootvnode_mount = rootvnode_mount_pac | 0xfffff00000000000;
...
uint64_t getVnodeAtPathByChdir(const char *path) {
